...
class TMXMapInfo {
	...
	public function parseXMLFile(tmxFile : String) {
		var map : Xml = Xml.parse(this.pack.getFile(tmxFile).toString()).firstElement();

		
		var version = map.get("version");
		var orientationStd = map.get("orientation");
		
		var mapSize : Size = new Size();
		...
		for (e in elem.elements()) {
			switch(e.nodeName) {
				case "image" :
					...
					tileset.imageSize = new Size(tileset.sourceImageWidth, tileset.sourceImageHeight);
					if (this._tileSets.length != 0) {
						this._tileSets[this._tileSets.length - 1].lastGid = tileset.firstGid - 1;
					}
	...
	private function loadObjectGroup(xml : Xml) {
		...
		if (xml.get("opacity") != null) {
			objectGroup.opacity = Std.parseFloat(xml.get("opacity"));
		}
				...
				case "object" :
					...
					for (e in elem.elements()) {
						if (e.nodeName == "polyline") {
							...
							object.vertexs = new Array<Point>();
							
							var vertexs : String = e.get("points");
							for (v in vertexs.split(" ")) {
								var t = v.split(",");
								var point : Point = new Point(Std.parseFloat(t[0]), Std.parseFloat(t[1]));
								object.vertexs.push(point);
							}
						} else if (e.nodeName == "ellipse") {
	...
	//public function parseJsonFile(tmxFile : String) {
		//var map = Json.parse(this.pack.getFile(tmxFile).toString());
		//
		//var version = map.version;
		//var orientationStd = map.orientation;
		//
		//var mapSize : Size = new Size();
		//mapSize.width = map.width;
		//mapSize.height = map.height;
		//this.setMapSize(mapSize);
		//
		//mapSize = new Size();
		//mapSize.width = map.tilewidth;
		//mapSize.height = map.tileheight;
		//this.setTileSize(mapSize);
		//
		//
		//switch (orientationStd) {
		...
		//var indexOfTileset : Int = 0;
		//
		//this.loadTileset(map.tilesets);
		//
		//for (e in map.layers) {
			//if (e.type == "tilelayer") {
				//this.loadLayer(e);
				//idx++;
			//} else if (e.type = "objectgroup") {
				//this.loadObjectGroup(e);
			//}
			//
			//
		//}
		//this.loadProperties(map.properties);
		//
		//for (elem in map.elements()) {
			//switch (elem.nodeName) {
				//case "tileset" : 
	...
	//}
	//
	//private function loadPropertiesJson(elem : Dynamic) {
		//for (e in Reflect.fields(elem)) {
			//switch(e.nodeName) {
				//case "property" :
					//var map : Map<String, String> = new Map<String, String>();
					//map[e.get("name")] = e.get("value");
	...
	//
	//
	//private function loadTilesetJson(elem : Dynamic) {
		//
		//
		//for (i in elem) {
			//var tileset : TMXTilesetInfo = new TMXTilesetInfo();
			//tileset.name = i.name;
			//tileset.firstGid = i.firstgid;
			//var marginStr = i.margin;
			//if (marginStr == null) {
				//marginStr = 0;
			//}
			//
			//var spacingStr = i.spacing;
			//if (spacingStr == null) {
				//spacingStr = 0;
			//}
			//
			//tileset.margin = marginStr;
			//tileset.spacing = spacingStr;
			//var tilesetSize : Size = new Size();
			//tilesetSize.width = i.tilewidth;
			//tilesetSize.height = i.tileheight;
			//tileset._tileSize = tilesetSize;
			//
			//var imgSource : String = i.image;
			//imgSource = StringTools.replace(imgSource, "\\", "");
			//imgSource = this._resources + "/" + imgSource;
			//tileset.sourceImage = imgSource;
			//
			//tileset.texture = this.pack.getTexture(tileset.sourceImage);
			//tileset.sourceImageWidth = i.imagewidth;
			//tileset.sourceImageHeight = i.imageheight;
			//tileset.imageSize = new Size(tileset.sourceImageWidth, tileset.sourceImageHeight);
			//if (this._tileSets.length != 0) {
				//this._tileSets[this._tileSets.length - 1].lastGid = tileset.firstGid - 1;
			//}
			...
			//
			//
			//var tileproperties = i.tileproperties;
			//var info = this._tileSets[this._tileSets.length - 1];
			//
			//
			//for (n in Reflect.fields(i.tileproperties)) {
				//var id = Reflect.field(i.tileproperties, n);
				//if (id == null) {
					//id = 0;
				//}
				//this.setParentGID(info.firstGid + id);
				...
				//var dict : Map<String, String> = new Map<String, String>();
				//
				//for (j in Reflect.fields(i.tileproperties.id)) {
					//var name = j;
					//var value = i.tileproperties.id.j;
					//dict.set(name, value);
				//}
				//
				//this._tileProperties.set(this.getParentGID(), dict);
	...
	//}
	//
	//private function loadLayerJson(elem : Dynamic) {
		//var layer : TMXLayerInfo = new TMXLayerInfo();
		//layer.name = elem.name;
		//layer.idx = this.idx;
		//
		//var layerSize : Size = new Size();
		//layerSize.width = elem.width;
		//layerSize.height = elem.height;
		//layer._layerSize = layerSize;
		//
		//var visible = elem.visible;
		...
		//}
		//
		//var opacity : Float = elem.opacity;
		//if (opacity == null) {
			//opacity = 1;
		//}
		//
//
		//layer._opacity = Std.int(255 * opacity);
		//
		//var x = Std.parseFloat(elem.get("x"));
		//var y = Std.parseFloat(elem.get("y"));
		//
		...
		//
		//var nodeValue :String = "";
		//loadLayerPros(e.properties, layer);
		//loadData(e.data, layer);
	//
		//this.setLayers(layer);
	//}
	//
	//private function loadDataJson(__data : Array<Dynamic>, layer : TMXLayerInfo) {
		//layer._tiles = _data;
	//}
	//
	//private function loadLayerProsJson(__pros : Dynamic, layer : TMXLayerInfo) {
		//for (elem in Reflect.fields(__pros)) {
			//layer.setProperties(elem, Reflect.field(__pros, elem));
		//}
	//}
	//
	//private function loadObjectGroupJson(__object : Dynamic) {
		//var objectGroup = new TMXObjectGroup();
		//objectGroup.setGroupName(xml.get("name"));
		//
		//var x : Float;
		...
		//for (o in __object.objects) {
			...
			//for (p in Reflect.fields(o.properties)) {
				//object.properties.set(p, Reflect.field(o,properties, p));
			//}
			//
			//objectGroup.setObjects(object);
		//}
